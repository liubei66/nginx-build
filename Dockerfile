...
RUN set -eux; \
    ...
    mkdir -p ${BORINGSSL_SRC_DIR}/include/openssl && \
    touch ${BORINGSSL_SRC_DIR}/include/openssl/ssl.h ${BORINGSSL_SRC_DIR}/include/openssl/opensslv.h
...
RUN set -eux; \
    ...
    ./configure \
        ...
        --with-cc-opt="-I${LUAJIT_INC} -I${BORINGSSL_SRC_DIR}/include -x c -I/usr/local/zstd-pic/include -I/usr/local/include -I${MODULE_BASE_DIR}/quickjs -Wno-error -Wno-deprecated-declarations -fPIC" \
        --with-ld-opt="-L${LUAJIT_LIB} -L${BORINGSSL_SRC_DIR}/build/ssl -L${BORINGSSL_SRC_DIR}/build/crypto -L/usr/local/lib -L/usr/local/zstd-pic/lib -L${MODULE_BASE_DIR}/quickjs -Wl,-rpath,${LUAJIT_LIB}:/usr/local/lib:/usr/local/zstd-pic/lib -lssl -lcrypto -lstdc++ -lzstd -lquickjs -lluajit-5.1 -lz -lpcre2-8 -ljemalloc -lpthread -Wl,-Bsymbolic-functions" \
...
LABEL description="Nginx ${NGINX_VERSION} with BoringSSL + custom modules + PCRE2 JIT + Jemalloc + HTTP3/QUIC + TLS Dynamic Size (Debian)"

# 安装运行时依赖，配置系统源、用户与目录权限
RUN set -eux; \
    apt-get update && apt-get install -y --no-install-recommends \
        ...
        iproute2 procps curl lsof dnsutils net-tools less jq vim wget htop libatomic1 g++-runtime; \
    ...
    echo "/usr/local/zstd-pic/lib" >> /etc/ld.so.conf.d/nginx.conf && ldconfig
